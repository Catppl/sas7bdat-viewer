...
class MergeDatasetsEngine:
    ...
    def validate(
        ...
        left_variables = {variable.name.casefold(): variable for variable in left.metadata.variables}
        right_variables = {variable.name.casefold(): variable for variable in right.metadata.variables}
        resolved: list[str] = []
        for requested in config.by_variables:
            ...
            if key not in left_variables or key not in right_variables:
                ...
                raise ValueError(f'BY variable "{requested}" is missing from {missing} dataset.')
    ...
    @staticmethod
    def _attach(connection: sqlite3.Connection, alias: str, handle: DatasetHandle) -> None:
        connection.execute(f"ATTACH DATABASE ? AS {alias}", (_source_uri(handle.database_path),))
    ...
    def _join_condition(
        ...
        left_by = {variable.name.casefold(): variable for variable in left.metadata.variables}
        right_by = {variable.name.casefold(): variable for variable in right.metadata.variables}
        parts = []
        for name in by_variables:
            ...
            valid_left = self._valid_value("l", left_variable.name, side=left_variable.kind)
            valid_right = self._valid_value("r", right_variable.name, side=right_variable.kind)
            parts.append(f"({valid_left} AND {valid_right} AND {left_column} = {right_column})")
        return " AND ".join(parts)

    @staticmethod
    def _key_columns(alias: str, variables: tuple[str, ...]) -> str:
        return ", ".join(f"{alias}.{quote_identifier(variable)}" for variable in variables)
    ...
    def inspect(
        ...
        try:
            ...
            left_dupes = self._duplicate_key_count(connection, "left_db", by_variables, left.metadata)
            right_dupes = self._duplicate_key_count(connection, "right_db", by_variables, right.metadata)
            left_by = {variable.name.casefold(): variable for variable in left.metadata.variables}
    ...
    def run(
        ...
        try:
            ...
            left_by = {variable.name.casefold(): variable for variable in left.metadata.variables}
            right_by = {variable.name.casefold(): variable for variable in right.metadata.variables}
            resolved_left_by = tuple(left_by[name.casefold()].name for name in by_variables)
            condition = self._join_condition(left, right, by_variables)
            select: list[str] = []
            ...
            for variable in left.metadata.variables:
                if variable.name.casefold() in by_set:
                    continue
                select.append(f"l.{quote_identifier(variable.name)} AS {quote_identifier(variable.name)}")
            for source_name, output_name, _kind in right_mapping:
                select.append(f"r.{quote_identifier(source_name)} AS {quote_identifier(output_name)}")
            ...
            if config.join_type == "left":
            ...
            elif sqlite3.sqlite_version_info >= (3, 39, 0):
                queries = (
                    insert_prefix
                    + select_sql
                    + " FROM left_db.dataset AS l FULL OUTER JOIN right_db.dataset AS r ON "
                    + condition,
                )
            else:
                # FULL OUTER JOIN became available in SQLite 3.39.  The
                # equivalent two LEFT JOINs keep older Python/SQLite builds
                # functional without touching either source database.
                right_only_query = (
                    insert_prefix
                    + select_sql
                    + " FROM right_db.dataset AS r LEFT JOIN left_db.dataset AS l ON "
                ...
                queries = (left_join_query, right_only_query)
            notify("Joining source datasets…")
            for query in queries:
                connection.execute(query)
            connection.commit()
            summary = self._result_summary(connection, left, right, by_variables)
    ...
    def _result_summary(
        ...
        # The result schema normally uses this exact name.  Resolve a suffixed
        # name if a source variable already occupied it.
        columns = {row[1].casefold(): row[1] for row in connection.execute("PRAGMA table_info(dataset)")}
        status_name = columns.get(
            "_merge_status",
            next(
                value
                for key, value in columns.items()
                if key.startswith("_merge_status")
            ),
        )
        ...
        counts = {
            ...
            for row in connection.execute(
                f"SELECT {quote_identifier(status_name)}, count(*) FROM dataset "
                f"GROUP BY {quote_identifier(status_name)}"
            )
        }
        return MergeSummary(
            left.metadata.row_count,
