...
class AeTableDenominator:
    ...
    # (for example, TRT01A versus TRTA in an AE source dataset).
...
class AeTableConfig:
    ...
    def validate(
        self, source: DatasetMetadata, population: DatasetMetadata | None = None
    ) -> None:
        ...
        for name, label in (
            (self.soc_variable, "SOC variable"),
            (self.pt_variable, "PT variable"),
            ...
            if not name or name.casefold() not in fields:
                raise ValueError(
                    f'{label} "{name}" does not exist in the source dataset.'
                )
        if self.denominator.type == "population":
            ...
            for name, label in (
                ...
                if source_variable.kind != pfields[name.casefold()].kind:
                    raise ValueError(
