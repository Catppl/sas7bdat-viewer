...
def _missing(value, variable):
    return value is None or (
        variable.kind == "character" and (value == "" or value is None)
    )
...
def _key(value):
    return json.dumps(
        _canon(value), ensure_ascii=False, separators=(",", ":"), default=str
    )
...
def _and(*parts):
    ...
    return (
        (
            " AND ".join(f"({sql})" for sql, _ in active),
            tuple(v for _, p in active for v in p),
        )
        if active
        else ("", ())
    )
...
class AeTableEngine:
    ...
    def resolve_treatment_levels(self, source, config, population=None):
        ...
        handles = [
            (
                source,
                config.dataset_filter.sql,
                config.dataset_filter.parameters,
                treatment,
            )
        ]
        if config.denominator.type == "population" and population is not None:
            ...
            pt = self._fields(population.metadata)[population_treatment.casefold()]
            handles.append(
                (
                    population,
        ...
        for handle, sql, params, variable in handles:
            ...
            with closing(self._connection(handle)) as conn:
                for (value,) in conn.execute(
                    f"SELECT DISTINCT {quote_identifier(variable.name)} FROM dataset{where}",
                    params,
                ):
                    value = _canon(value)
                    if _missing(value, variable):
                        raise MissingTreatmentError(
                            "Missing treatment values were found. Modify the Dataset Filter or Population WHERE before running."
                        )
                    key = _key(value)
                    if key not in seen:
                        seen.add(key)
                        values.append((key, value, _display(value)))
        values.sort(key=lambda item: str(item[2]).casefold())
        return values

    def _aggregate(
        self, handle, config_filter, soc, pt, treatment, subject, missing_policy
    ):
        where = f" WHERE {config_filter.sql}" if config_filter.sql else ""
        sql = (
            f"SELECT {quote_identifier(treatment.name)}, {quote_identifier(soc.name)}, {quote_identifier(pt.name)}, {quote_identifier(subject.name)} "
            f"FROM dataset{where}"
        )
        any_counts = defaultdict(set)
        soc_counts = defaultdict(set)
        pt_counts = defaultdict(set)
        with closing(self._connection(handle)) as conn:
            for treatment_value, soc_value, pt_value, subject_value in conn.execute(
                sql, config_filter.parameters
            ):
                treatment_value, soc_value, pt_value = map(
                    _canon, (treatment_value, soc_value, pt_value)
                )
                if _missing(treatment_value, treatment):
                    raise MissingTreatmentError(
                        "Missing treatment values were found. Modify the Dataset Filter before running."
                    )
                if _missing(subject_value, subject):
                    continue
                tkey = _key(treatment_value)
                any_counts[tkey].add(subject_value)
                any_counts[TOTAL_KEY].add(subject_value)
                soc_value = _hierarchy_value(soc_value, soc, missing_policy)
                if soc_value is None:
                    continue
                skey = _key(soc_value)
                soc_counts[(skey, tkey)].add(subject_value)
                soc_counts[(skey, TOTAL_KEY)].add(subject_value)
                pt_value = _hierarchy_value(pt_value, pt, missing_policy)
                if pt_value is None:
                    continue
                pt_counts[(skey, _key(pt_value), tkey)].add(subject_value)
                pt_counts[(skey, _key(pt_value), TOTAL_KEY)].add(subject_value)
    ...
    def _denom(self, source, population, config, treatment, subject):
        if config.denominator.type == "population":
            ...
            treatment = self._fields(population.metadata)[
                population_treatment.casefold()
            ]
        ...
        with closing(self._connection(handle)) as conn:
            for value, sid in conn.execute(
                f"SELECT {quote_identifier(treatment.name)}, {quote_identifier(subject.name)} FROM dataset{where}",
                compiled.parameters,
            ):
                value = _canon(value)
                ...
                if _missing(sid, subject):
                    continue
                counts[_key(value)].add(sid)
                counts[TOTAL_KEY].add(sid)
    ...
    def run(
        ...
        if not source.cache_complete or (
            population is not None and not population.cache_complete
        ):
        ...
        soc, pt = (
            fields[config.soc_variable.casefold()],
            fields[config.pt_variable.casefold()],
        )
        treatment, subject = (
            fields[config.treatment_variable.casefold()],
            fields[config.subject_id_variable.casefold()],
        )
        notify("Resolving treatment levels…")
        levels = self.resolve_treatment_levels(source, config, population)
        any_counts, soc_counts, pt_counts = self._aggregate(
            source,
            ...
            treatment,
            subject,
            config.hierarchy_missing_policy,
        )
        ...
        pt_labels = {
            (skey, pkey): _display(json.loads(pkey)) for skey, pkey, _t in pt_counts
        }
        soc_order = sorted(
            soc_labels,
            key=lambda s: (
                -len(soc_counts.get((s, TOTAL_KEY), ())),
                soc_labels[s].casefold(),
            ),
        )
        pt_order = {
            s: sorted(
                [p for ss, p in pt_labels if ss == s],
                key=lambda p: (
                    -len(pt_counts.get((s, p, TOTAL_KEY), ())),
                    pt_labels[(s, p)].casefold(),
                ),
            )
            for s in soc_order
        }
        rows = []
        if config.include_any_ae:
            rows.append(
                {
                    "row_type": "any",
                    "soc": None,
                    "pt": None,
                    "item": config.any_ae_label,
                    "indent": 0,
                }
            )
        for s in soc_order:
        ...
        try:
            for row in rows:
                if row["row_type"] == "any":
                    counts = any_counts
                elif row["row_type"] == "soc":
                    counts = {
                        (t): vals
                        for (s, t), vals in soc_counts.items()
                        if s == _key(row["soc"])
                    }
                else:
                    counts = {
                        (t): vals
                        for (s, p, t), vals in pt_counts.items()
                        if s == _key(row["soc"]) and p == _key(row["pt"])
                    }
                writer.add_row(
                    row,
                    counts,
                    denom,
                    levels,
                    config.dataset_filter_text,
                    config.denominator.type,
                )
