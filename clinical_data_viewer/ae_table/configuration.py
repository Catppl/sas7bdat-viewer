...
def _input(source: DatasetHandle) -> dict[str, object]:
    if source.kind not in {"sas", "merge"}:
        raise ValueError(
            f'AE Table configuration does not support source kind "{source.kind}".'
        )
    merge = source.kind == "merge"
    suffix = source.source_path.suffix.lower().lstrip(".")
    if not merge and suffix not in {"sas7bdat", "xpt"}:
        raise ValueError(
            f'AE Table configuration does not support source format "{suffix}".'
        )
    return {
        "kind": source.kind,
        "format": "merge" if merge else suffix,
        "dataset": source.metadata.name,
        "source_path": None if merge else str(source.source_path),
...
def build_ae_table_configuration(
    ...
    return {
        ...
        "hierarchy": {
            "type": "soc_pt",
            "soc_variable": config.soc_variable,
            "pt_variable": config.pt_variable,
            "missing": {"policy": config.hierarchy_missing_policy, "label": "Uncoded"},
        },
        ...
        "rows": {
            "include_any_ae": config.include_any_ae,
            "any_ae_label": config.any_ae_label,
        },
        "sort": {
            "soc": {
                "by": "total_frequency",
...
def write_ae_table_configuration(
    path: Path,
    source: DatasetHandle,
    config: AeTableConfig,
    population: DatasetHandle | None = None,
    ...
    value = build_ae_table_configuration(
        source, config, population, resolved_treatment_levels, resolved_hierarchy
    )
