...
class AeTableBuilder(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.treatment.currentIndexChanged.connect(self._source_treatment_changed)
        form.addRow("Treatment variable", self.treatment)
        subject = QLabel("USUBJID")
        subject.setTextInteractionFlags(Qt.TextSelectableByMouse)
        form.addRow("Subject ID", subject)
        self.soc = QComboBox()
        form.addRow("SOC variable", self.soc)
        self.pt = QComboBox()
        form.addRow("PT variable", self.pt)
        ...
        filters = QGroupBox("Dataset Filter")
        f = QVBoxLayout(filters)
        self.dataset_filter = QPlainTextEdit()
        self.dataset_filter.setPlaceholderText('e.g. TRTEMFL = "Y"')
        self.dataset_filter.setMaximumHeight(78)
        self.dataset_filter.textChanged.connect(
            lambda: setattr(
                self, "_filter_text", self.dataset_filter.toPlainText().strip()
            )
        )
        f.addWidget(self.dataset_filter)
        layout.addWidget(filters)
        denom = QGroupBox("Denominator")
        df = QFormLayout(denom)
        ...
        self.denominator_type.currentIndexChanged.connect(self._sync_denominator)
        df.addRow("Type", self.denominator_type)
        self.adsl = QComboBox()
        self.adsl.currentIndexChanged.connect(self._adsl_changed)
        row = QHBoxLayout()
        row.addWidget(self.adsl, 1)
        browse = QPushButton("Browse…")
        browse.clicked.connect(self.browse_adsl_requested)
        row.addWidget(browse)
        df.addRow("ADSL dataset", row)
        self.population_treatment = QComboBox()
        self.population_treatment.currentIndexChanged.connect(
            self._population_treatment_changed
        )
        df.addRow("ADSL treatment variable", self.population_treatment)
        self.population_where = QLineEdit()
        self.population_where.setPlaceholderText('e.g. SAFFL = "Y"')
        df.addRow("Population WHERE", self.population_where)
        layout.addWidget(denom)
        options = QGroupBox("Display")
        of = QFormLayout(options)
        self.include_any = QCheckBox("Include Any AE row")
        self.include_any.setChecked(True)
        of.addRow(self.include_any)
        self.any_label = QLineEdit("Any AE")
        of.addRow("Any AE label", self.any_label)
        self.include_total = QCheckBox("Include Total column")
        self.include_total.setChecked(True)
        of.addRow(self.include_total)
        self.percent_digits = QSpinBox()
        self.percent_digits.setRange(0, 4)
        self.percent_digits.setValue(1)
        of.addRow("Percent digits", self.percent_digits)
        ...
        self.run_button = QPushButton("Run AE Table")
        self.run_button.setDefault(True)
        self.run_button.clicked.connect(self._run)
        buttons.addWidget(self.run_button, 1)
        outer.addLayout(buttons)
        self._sync_denominator()
    ...
    def set_dataset(
        ...
        if metadata is not None and metadata is not self._metadata:
            ...
            for combo, preferred in (
                (self.treatment, ("TRT01A", "TRT01P", "TRTA", "TRT01AN", "TRTAN")),
                (self.soc, ("AEBODSYS",)),
                (self.pt, ("AEDECOD",)),
            ):
                combo.clear()
        ...
        self.sas_code_button.setToolTip(
            ...
            else "Generate reusable SAS code from the current AE configuration."
        )
    ...
    def set_adsl_sources(self, datasets):
        ...
        elif datasets and not self._adsl_user_selected:
            for index in range(self.adsl.count()):
                tab = self.adsl.itemData(index)
                if (
                    getattr(getattr(tab, "handle", None), "metadata", None)
                    and tab.handle.metadata.name.casefold() == "adsl"
                ):
    ...
    def _adsl_changed(self, _index):
        if not self.adsl.signalsBlocked():
            self._adsl_user_selected = True
            self._refresh_population_treatment_variables()

    def _population_treatment_changed(self, _index):
        if not self.population_treatment.signalsBlocked():
            self._population_treatment_user_selected = True

    def _source_treatment_changed(self, _index):
        if (
            not self.treatment.signalsBlocked()
            and not self._population_treatment_user_selected
        ):
            self._refresh_population_treatment_variables()

    def _refresh_population_treatment_variables(self):
        selected = (
    ...
    def _sync_denominator(self):
        population = self.denominator_type.currentData() == "population"
        self.adsl.setEnabled(population and not self._busy)
        self.population_treatment.setEnabled(population and not self._busy)
        self.population_where.setEnabled(population and not self._busy)

    def set_busy(self, busy: bool, message: str = ""):
        self._busy = busy
        available = not busy and self._metadata is not None
        self.run_button.setEnabled(available)
        self.sas_code_button.setEnabled(available and self._source_kind == "sas")
        self._sync_denominator()
    ...
    def _selection(self):
        ...
        population = (
            ...
            if self.denominator_type.currentData() == "population"
            else None
        )
        return AeTableBuilderSelection(
            self._filter_text,
            self.treatment.currentText(),
            self.soc.currentText(),
            self.pt.currentText(),
            self.denominator_type.currentData(),
            population,
            self.population_treatment.currentText(),
            self.population_where.text().strip(),
            self.include_any.isChecked(),
            self.any_label.text().strip() or "Any AE",
            self.include_total.isChecked(),
            self.percent_digits.value(),
            self.missing_policy.currentData(),
        )
