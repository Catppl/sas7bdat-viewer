...
from clinical_data_viewer.ae_table import (
    AeTableConfig,
    AeTableDenominator,
    AeTableEngine,
...
def handle(root, name, variables, rows):
    directory = root / (name + "-cache")
    directory.mkdir()
    db = directory / "dataset.sqlite"
    defs = ", ".join(
        f'"{v.name}" {"REAL" if v.kind == "numeric" else "TEXT"}' for v in variables
    )
    with closing(sqlite3.connect(db)) as c:
        ...
        c.execute("INSERT INTO cache_info VALUES (?,?,1)", (len(rows), len(rows)))
        c.commit()
    src = root / f"{name}.sas7bdat"
    src.touch()
    return DatasetHandle(
        src,
        directory / src.name,
        db,
        DatasetMetadata(name.upper(), len(rows), tuple(variables)),
        len(rows),
        True,
    )
...
class AeTableTests(unittest.TestCase):
    def test_missing_treatment_blocks_calculation(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            source = handle(root, "adae", variables, (("S1", None, "GI", "Nausea"),))
            with self.assertRaises(MissingTreatmentError):
                AeTableEngine(TempManager(root / "temp")).run(
                    source, AeTableConfig("SOC", "PT", "TRT")
                )

    def test_subject_variable_is_fixed_to_usubjid(self):
        variables = tuple(
            VariableMetadata(n) for n in ("USUBJID", "TRT", "SOC", "PT", "SUBJ")
        )
        metadata = DatasetMetadata("ADAE", 0, variables)
        with self.assertRaisesRegex(ValueError, "USUBJID"):
            AeTableConfig("SOC", "PT", "TRT", subject_id_variable="SUBJ").validate(
                metadata
            )
    ...
    def test_uncoded_missing_soc_and_pt_are_hierarchy_levels(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            source = handle(root, "adae", variables, rows)
            config = AeTableConfig(
                "SOC", "PT", "TRT", hierarchy_missing_policy="uncoded"
            )
            ...
            with closing(sqlite3.connect(result.database_path)) as c:
                items = [
                    r[0].replace("\u00a0", "")
                    for r in c.execute(
                        'SELECT "ITEM" FROM dataset ORDER BY _source_row'
                    )
                ]
                uncoded_soc = c.execute(
                    'SELECT "TRT_1" FROM dataset WHERE "ITEM" = ?', ("Uncoded",)
                ).fetchone()[0]
            self.assertEqual(
                items, ["Any AE", "GI", "Nausea", "Uncoded", "Uncoded", "Headache"]
            )
            ...
            self.assertEqual(
                configuration["hierarchy"]["missing"],
                {"policy": "uncoded", "label": "Uncoded"},
            )
    ...
    def test_denominator_drilldown_never_contains_soc_pt(self):
        source_vars = tuple(
            VariableMetadata(n) for n in ("USUBJID", "TRT", "SOC", "PT")
        )
        pop_vars = tuple(VariableMetadata(n) for n in ("USUBJID", "TRT", "SAFFL"))
        source_meta = DatasetMetadata("ADAE", 0, source_vars)
        pop_meta = DatasetMetadata("ADSL", 0, pop_vars)
        ...
        for denominator_type, metadata, denominator in (
            ...
            (
                "population",
                pop_meta,
                AeTableDenominator("population", pop_filter, 'SAFFL = "Y"'),
            ),
        ):
            config = AeTableConfig(
                "SOC",
                "PT",
                "TRT",
                dataset_filter=source_filter,
                dataset_filter_text='TRT = "A"',
                denominator=denominator,
            )
            normal_cell = AeTableCell("pt", "GI", "Nausea", "A")
            sql, _params = build_cell_filter(
                metadata, config, normal_cell, denominator=True
            )
            ...
            self.assertEqual(
                sql.count('"TRT" ='), 1 if denominator_type == "same_universe" else 0
            )
    ...
    def test_hierarchy_distinct_counts_and_sorting(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            config = AeTableConfig(
                ...
                "TRT",
                dataset_filter=source_filter,
                dataset_filter_text='TRT in ("A", "B")',
            )
            result = AeTableEngine(TempManager(root / "temp")).run(source, config)
            with closing(sqlite3.connect(result.database_path)) as c:
                items = [
                ...
                gi = c.execute(
                    'SELECT "TRT_1" FROM dataset WHERE "ITEM" = ?', ("GI",)
                ).fetchone()[0]
                nausea = c.execute(
                    'SELECT "TRT_1" FROM dataset WHERE "ITEM" LIKE ?', ("%Nausea",)
                ).fetchone()[0]
            self.assertEqual(
                items,
                [
            ...
            self.assertEqual(
                configuration["dataset_filter"]["text"], 'TRT in ("A", "B")'
            )
    ...
    def test_population_filter_is_independent_and_merge_contract(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            config = AeTableConfig(
                ...
                "PT",
                "TRT",
                denominator=AeTableDenominator("population", pf, 'SAFFL = "Y"'),
            )
            ...
            cfg = build_ae_table_configuration(
                merge, AeTableConfig("SOC", "PT", "TRT"), resolved_treatment_levels=[]
            )
            self.assertEqual(
                (cfg["input"]["kind"], cfg["input"]["format"]), ("merge", "merge")
            )

    def test_population_denominator_can_use_a_different_treatment_variable(self):
        source_variables = tuple(
            VariableMetadata(name) for name in ("USUBJID", "TRTA", "SOC", "PT")
        )
        population_variables = tuple(
            VariableMetadata(name) for name in ("USUBJID", "TRT01A", "SAFFL")
        )
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = AeTableConfig(
                ...
                "PT",
                "TRTA",
                denominator=AeTableDenominator(
                    "population",
                    population_filter,
                    'SAFFL = "Y"',
                    "TRT01A",
            ...
            result = AeTableEngine(TempManager(root / "temp")).run(
                source, config, population
            )
            with closing(sqlite3.connect(result.database_path)) as connection:
                values = connection.execute(
                    'SELECT "TRT_1", "TRT_2" FROM dataset WHERE "ITEM" = ?',
                    ("GI",),
                ).fetchone()
            self.assertEqual(values, ("1 (100.0)", "1 (100.0)"))
            configuration = json.loads(
                result.configuration_path.read_text(encoding="utf-8")
            )
            self.assertEqual(configuration["treatment"]["variable"], "TRTA")
            self.assertEqual(
                configuration["denominator"]["population"]["treatment_variable"],
                "TRT01A",
            )

    def test_population_drilldown_uses_the_population_treatment_variable(self):
        population_variables = tuple(
            VariableMetadata(name) for name in ("USUBJID", "TRT01A", "SAFFL")
        )
        ...
        config = AeTableConfig(
            ...
            "TRTA",
            denominator=AeTableDenominator(
                "population", population_filter, 'SAFFL = "Y"', "TRT01A"
            ),
        )
        sql, parameters = build_cell_filter(
            DatasetMetadata("ADSL", 0, population_variables),
            config,
            AeTableCell("soc", "GI", None, "A"),
            denominator=True,
        )
        self.assertIn('"TRT01A" = ?', sql)
        self.assertNotIn('"TRTA"', sql)
        self.assertEqual(parameters, ("Y", "A"))
    ...
    def test_total_recomputes_distinct_subjects_not_arm_sum(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with closing(sqlite3.connect(result.database_path)) as c:
                row = c.execute(
                    'SELECT "TRT_1", "TRT_2", "TOTAL" FROM dataset WHERE "ITEM" = ?',
                    ("Any AE",),
                ).fetchone()
    ...
    def test_optional_any_total_and_decimal_display(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with closing(sqlite3.connect(result.database_path)) as c:
                ...
                item = c.execute(
                    'SELECT "ITEM", "TRT_1" FROM dataset ORDER BY _source_row LIMIT 1'
                ).fetchone()
    ...
    def test_numeric_treatment_and_long_result_order(self):
        variables = (
            ...
            VariableMetadata("TRT", kind="numeric"),
            VariableMetadata("SOC"),
            VariableMetadata("PT"),
        )
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            source = handle(
                ...
                "adae",
                variables,
                (("S1", 2, "GI", "Nausea"), ("S2", 1, "GI", "Nausea")),
            )
            manager = TempManager(root / "temp")
            result = AeTableEngine(manager).run(
                source, AeTableConfig("SOC", "PT", "TRT")
            )
            config = json.loads(result.configuration_path.read_text(encoding="utf-8"))
            self.assertEqual(
                [level["value"] for level in config["treatment"]["resolved_levels"]],
                [1, 2],
            )
            ...
            with closing(sqlite3.connect(long_result.database_path)) as c:
                orders = c.execute(
                    "SELECT ROW_ORDER, TRT_ORDER, TRT FROM dataset ORDER BY ROW_ORDER, TRT_ORDER"
                ).fetchall()
    ...
    def test_merge_source_runs_ae_table_engine(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            base = handle(root, "merged", variables, (("S1", "A", "GI", "Nausea"),))
            merge = DatasetHandle(
                base.source_path,
                base.temporary_path,
                base.database_path,
                base.metadata,
                base.cached_row_count,
                True,
                kind="merge",
            )
            result = AeTableEngine(TempManager(root / "temp")).run(
                merge, AeTableConfig("SOC", "PT", "TRT")
            )
            self.assertEqual(result.kind, "ae_table")
            config = json.loads(result.configuration_path.read_text(encoding="utf-8"))
            self.assertEqual(
                (config["input"]["kind"], config["input"]["format"]), ("merge", "merge")
            )
