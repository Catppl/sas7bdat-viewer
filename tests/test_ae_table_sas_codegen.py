...
def configuration(
    *,
    ...
    value = {
        "type": "ae_soc_pt_table",
        "version": 1,
        "input": {
            "kind": "sas",
            "format": "sas7bdat",
            "dataset": "ADAE",
            "source_path": r"C:\data\adae.sas7bdat",
            "source_directory": r"C:\data",
        },
        "variables": variables,
        "dataset_filter": {
            "language": "sas_like",
        ...
        "resolved_hierarchy": [
            {"row_type": "soc", "soc": "OLD", "pt": None, "item": "OLD", "indent": 0}
        ],
    ...
    if denominator == "population":
        value["denominator"]["population"] = {
            "input": {
                ...
                "format": "sas7bdat",
                "dataset": "ADSL",
                "source_path": r"C:\data\adsl.sas7bdat",
                "source_directory": r"C:\data",
            },
            "variables": {
                "USUBJID": variables["USUBJID"],
                "TRT01A": variables["TRT01A"],
            },
            "filter": {
                "language": "sas_like",
                "text": 'SAFFL = "Y"',
...
class AeSasGeneratorTests(unittest.TestCase):
    ...
    def test_population_can_use_a_different_treatment_variable(self):
        cfg = configuration(denominator="population")
        source_treatment = cfg["variables"].pop("TRT01A")
        cfg["variables"]["TRTA"] = source_treatment
        cfg["treatment"]["variable"] = "TRTA"
        cfg["denominator"]["population"]["treatment_variable"] = "TRT01A"
        code = SasAeTableGenerator().generate(cfg)
        self.assertIn("if missing(TRTA) then do;", code)
        self.assertIn("_trt = strip(TRTA);", code)
        self.assertIn("if missing(TRT01A) then do;", code)
        self.assertIn("_trt = strip(TRT01A);", code)
        self.assertIn("Treatment: TRTA", code)
        self.assertIn("Population Treatment: TRT01A", code)
        self.assertIn(
            "Prepare population denominator using TRT01A",
            code,
        )

    def test_options_and_final_columns(self):
        code = SasAeTableGenerator().generate(
            configuration(include_any=False, include_total=False, digits=2)
        )
    ...
    def test_rejects_merge_and_invalid_numerator(self):
        ...
        cfg = configuration()
        cfg["calculation"]["numerator"] = "records"
