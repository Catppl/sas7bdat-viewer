...
14. 点击表头文字区域确认仍排序；点击右侧 `▼` 确认只打开筛选。验证 Values 搜索、Select All、Missing、数值 Between、字符 Contains 和蓝色筛选标签。只选 `PARAMCD=ALB` 时 WHERE 必须是 `PARAMCD in ("ALB")`，没有多余外层括号且不得自动包含 missing；明确勾选 Missing 后才加入小写的 `or missing(PARAMCD)`。其他自动生成关键词也应使用小写。
...
17. 分别设置 PROC MEANS 的 Mean=2、SD=3、Median=1、Min/Max=0 位小数，同时修改置信水平和显示统计量；重启后确认每项设置恢复。n/N/NMISS 保持整数，显示四舍五入但计算值和源值不变。
18. 在行号上按 Ctrl 非连续选择 2–20 行并比较；浅黄色只能出现在所选行与差异列的交叉单元格，并应在蓝色整行选中状态下仍清晰可见；所选行的相同值列保持蓝色，未选中的其他行不得高亮，隐藏变量差异仍在 Analysis 面板列出。
