...
9. 退出并重启，确认 Filter History 可恢复；测试当前数据集/全局、回填、删除和清空。
...
14. 点击表头文字区域确认仍排序；点击右侧 `▼` 确认只打开筛选。验证 Values 搜索、Select All、Missing、数值 Between、字符 Contains 和蓝色筛选标签。
15. 同时应用手写 WHERE 与两个列头筛选，确认状态行数、Ctrl+F、PROC MEANS 和 CSV 都使用相同的最终结果。
...
18. 在行号上按 Ctrl 非连续选择 2–20 行并比较；差异列在表格中浅黄色高亮，隐藏变量差异仍在 Analysis 面板列出。
